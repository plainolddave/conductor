...
public class MongoMetadataDAOTest {
    ...
    // @Rule
    // TOD0
    // public ExpectedException thrown = ExpectedException.none(); xxx
    ...
    // // TODO thrown.expect(hasProperty("code", is(CONFLICT)));
    ...
    public void testRemoveNotExistingWorkflowDef() {
        NonTransientException applicationException = assertThrows(NonTransientException.class,
                () -> metadataDAO.removeWorkflowDef("test", 1));
        assertEquals("No such workflow definition: test version: 1",
    ...
    public void testWorkflowDefOperations() {
        ...
        all = metadataDAO.getAllWorkflowDefsLatestVersions();
        assertNotNull(all);
        assertEquals(1, all.size());
        assertEquals("test", all.get(0).getName());
    ...
    // // TODO thrown.expect(hasProperty("code", is(NOT_FOUND)));
