...
public class MongoQueueDAOTest {
    ...
    // TODO
    // @Rule
    // public ExpectedException expected = ExpectedException.none(); xxx
    ...
    public void pollMessagesTest() {
        ...
        assertEquals("Total queue size should have remained the same", totalSize,
        ...
        try {
            Query unpoppedQuery = new Query().addCriteria(
                    Criteria.where("queue_name").is(queueName).and("popped").is(false));
    ...
    public void pollDeferredMessagesTest() throws InterruptedException {
        ...
        List<String> firstPollMessageIds = messages.stream().map(Message::getId)
                .collect(Collectors.toList()).subList(0, firstPollSize + 1);
